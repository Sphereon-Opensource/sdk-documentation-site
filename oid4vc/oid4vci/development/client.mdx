...
const client = await OpenID4VCIClientV1_0_15.fromURI({
  uri: 'openid-credential-offer://?credential_offer=',
  kid: 'did:example:ebfeb1f712ebc6f1c276e12ec21#key-1',
  alg: Alg.ES256,
...
In v1.0-15, Credential Issuers can work with multiple Authorization Servers. The metadata includes an `authorization_servers` array:
```typescript
const metadata = await client.retrieveServerMetadata();

// Check if multiple authorization servers are supported
if (metadata.credentialIssuerMetadata?.authorization_servers) {
  console.log('Supported Authorization Servers:',
    metadata.credentialIssuerMetadata.authorization_servers);
}

// Important: When a Credential Offer includes an `authorization_server` parameter,
// the Wallet MUST verify it matches one of the values in the authorization_servers array
const offerAuthServer = client.credentialOffer?.grants?.['urn:ietf:params:oauth:grant-type:pre-authorized_code']?.authorization_server;
if (offerAuthServer && metadata.credentialIssuerMetadata?.authorization_servers) {
  const isValid = metadata.credentialIssuerMetadata.authorization_servers.includes(offerAuthServer);
  if (!isValid) {
    throw new Error('Authorization server in offer not supported by issuer');
  }
}
```
...
```typescript
const accessToken = await client.acquireAccessToken({ tx_code: '1234' });
 ...
 *       credential_identifiers: ['badge-2024-001']
 *     }
 *   ]
 * }
 */
...
const credentialResponse = await client.acquireCredentials({
  credential_configuration_id: 'OpenBadgeCredential', // Use the ID from metadata
...
The MetadataClientV1_0_15 checks the OpenID4VCI well-known location for the medata and existence of a token endpoint. If the
OpenID4VCI well-known location is not found, the OIDC/OAuth2 well-known locations will be tried:

Example:

```typescript
import { MetadataClientV1_0_15 } from '@sphereon/oid4vci-client';

const metadata = await MetadataClientV1_0_15.retrieveAllMetadataFromCredentialOffer(initiationRequestWithUrl);
...
## New in v1.0.15: credential_identifier

In OID4VCI v1.0.15, the Token Response may include `authorization_details` with `credential_identifiers`. When present, these identifiers must be used in the Credential Request instead of `credential_configuration_id`:
```typescript
// If authorization_details with credential_identifiers was returned:
const credentialResponse = await client.acquireCredentials({
  credential_identifier: 'CivilEngineeringDegree-2023', // From Token Response
  proofCallbacks: callbacks,
  format: 'jwt_vc_json',
  alg: Alg.ES256K,
  kid: 'did:example:ebfeb1f712ebc6f1c276e12ec21#keys-1',
...
// Otherwise, use credential_configuration_id:
const credentialResponse = await client.acquireCredentials({
...
You can build/create a JWT yourself. You would still use the callbacks to sign the JWT.
Obtain a fresh c_nonce from the Nonce Endpoint before building the proof (see Nonce Endpoint section).
The c_nonce MUST be included in the proof as the `nonce` value. You can provide another nonce using
the `jti` property.
...
// Or in 2 steps using credential_configuration_id:
// const credentialRequest: CredentialRequest = await credentialRequestClient.createCredentialRequest(proofOpts, { credential_configuration_id: 'OpenBadgeCredential' })
...
## Nonce Endpoint (v1.0.15)

Version 1.0.15 introduces a dedicated **Nonce Endpoint** for obtaining fresh `c_nonce` values without requiring an access token. This is a significant change from previous versions where `c_nonce` was returned in the Token Response.

### Why the Nonce Endpoint?

The Nonce Endpoint allows wallets to:
- Obtain a nonce before making proof-of-possession
- Refresh nonces without making a full credential request
- Support better replay attack prevention

### Usage
```typescript
// Retrieve a nonce from the Nonce Endpoint (requires no authentication)
const nonceResponse = await client.retrieveNonce();
console.log(nonceResponse);
/**
 * {
 *   c_nonce: 'wKI4LT17ac15ES9bw8ac4'
 * }
 * Note: c_nonce_expires_in was removed in v15
 */

// Use this nonce when building your proof for the Credential Request
const proofInput = await ProofOfPossessionBuilder.fromJwt({
  jwt: {
    header: { alg: Alg.ES256, kid: 'did:example:123#key-1', typ: 'openid4vci-proof+jwt' },
    payload: {
      iss: clientId,
      aud: issuerIdentifier,
      iat: Math.floor(Date.now() / 1000),
      nonce: nonceResponse.c_nonce // Use the nonce from Nonce Endpoint
    }
  },
  callbacks
}).build();
...
const CREDENTIAL_OFFER_URI =
  ...
  'credential_offer=%7B%22credential_issuer%22:%22https://credential-issuer.example.com%22,%22credential_configuration_ids%22:%5B%22UniversityDegreeCredential%22%5D,%22grants%22:%7B%22urn:ietf:params:oauth:grant-type:pre-authorized_code%22:%7B%22pre-authorized_code%22:%22oaKazRN8I0IbtZ0C7JuMn5%22%7D%7D%7D';
...
**Note**: The `openid-credential-offer://` scheme is the correct scheme for v1.0-15. Legacy schemes like `openid-initiate-issuance://` with `credential_type` parameters indicate pre-v15 specifications.
