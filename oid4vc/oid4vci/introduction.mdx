...
In the OID4VCI Specification (Draft 15), a **Credential Offer** is a mechanism through which an issuer initiates the process of issuing Digital Credentials to a wallet or agent.
...
In a **by value** approach, the entire credential offer is contained directly within the message received by the wallet using the `credential_offer` parameter. This means the issuer provides all the necessary
...
In a **by reference** approach, the credential offer is represented by a URL using the `credential_offer_uri` parameter that points to the complete offer that resides on the issuer's server. The wallet
...
### Credential Configuration IDs and Credential Offers

The specification uses **credential_configuration_ids** to identify specific credential configurations offered by the issuer. These identifiers allow issuers to dynamically tailor the offer
based on predefined configurations stored in the `credential_configurations_supported` parameter of the server's metadata. For example:

- A specific **credential_configuration_id** identifies one of the keys in the `credential_configurations_supported` Credential Issuer metadata, which defines the types of credentials available for issuance, supported formats, cryptographic binding methods, and display properties.

When issuing credentials:

- A **by value** offer directly embeds the `credential_configuration_ids` array to indicate which credentials are being offered.
- A **by reference** offer retrieves the credential offer from a URL, which then contains the `credential_configuration_ids` for the wallet to process.
---
...
6. When cryptographic binding is required, the wallet may need to obtain a `c_nonce` value for proof of possession. The wallet can receive a `c_nonce` from the Token Response, from error responses at the Credential Endpoint, or by calling the optional Nonce Endpoint to get a fresh nonce value.
...
* The issuer generates a **pre-authorized_code** (using grant type `urn:ietf:params:oauth:grant-type:pre-authorized_code`) and shares it with the wallet (e.g., via QR code or deep link).
...
* To prevent replay attacks, a **Transaction Code** (tx_code) should be used, especially in cross-device scenarios. The Transaction Code is sent to the end-user via a separate channel (e.g., SMS, email) and must be entered by the user and sent by the wallet in the Token Request.
...
full [OID4VCI Draft 15 specification](https://openid.net/specs/openid-4-verifiable-credential-issuance-1_0-15.html).