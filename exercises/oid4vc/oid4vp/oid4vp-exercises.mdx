...
[API (swaggerhub)](https://app.swaggerhub.com/apis/SphereonInt/OID4VP/0.1.0#/Backend/create-authorization-request) to create a so-called Authentication Request by providing a `dcql query`.

You will need to provide a `queryId` for instance with value `PensionSdJwt` as path parameter, given every OID4VP Authorization Request will internally use a Presentation Definition (or DCQL query in the future)
...
In a lot of situations your wallet is on a mobile device, while you might be visiting the relying party website using another device like a tablet or desktop computer. This is the so called cross-device flow. Although new mechanisms are being developer whereby your mobile wallet can register with your browser, right now an Authorization Request is being conveyed using a QR code.
...
Take the outcome of Exercise 1 and copy the value of the `authRequestURI` field from the JSON response. So do not copy the whole JSON response. You should only copy the value that starts with `openid4vp://?request_uri=....` also minus the quotes in front and at the end.
...
The `create-authorization-request` endpoint we used above internally creates a session. The session contains all kind of information needed by the relying party; one of the important properties it has in the session is the `state`. The state can have the following values:

| state    | Description                                                                                                                                                                                 |
|---------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| authorization_request_created   | An Authorization Request is created. This is the initial state                                                                                                       |
| authorization_request_retrieved | When the wallet has retrieved the Authorization Request by reference using the URI provided in the link                                                              |
| authorization_response_received | When the Relying Party received the presentation(s) from the wallet                                                                                                  |
| authorization_response_verified | When the Relying Party has verified that the presentation(s) were cryptographically correct and were satisfying the credential queries (Presentation Exchange, DCQL) |
| error                           | An error occurred                                                                                                                                                    |

<Note>The above states are more or less in order.</Note>


## Exercise 4: Get the initial status
In this exercise we will retrieve the initial status of the credential offer. Without a wallet involved. The state will be `authorization_request_created`

Please use the `get-authorization-status` [API (docs)](/oid4vc/siop-oid4vp/api-reference/backend/get-authorization-status) or
[API (swaggerhub)](https://app.swaggerhub.com/apis/SphereonInt/OID4VP/0.1.0#/Backend/get-authorization-status) to check the progress of the session using the `correlationId` value you supplied during session creation. You could also create a new session as explained in the above exercises.
...
Please use the `get-authorization-status` [API (docs)](/oid4vc/siop-oid4vp/api-reference/backend/get-authorization-status) or
[API (swaggerhub)](https://app.swaggerhub.com/apis/SphereonInt/OID4VP/0.1.0#/Backend/get-authorization-status) to check the progress of the session using the `correlationId` value you supplied during session creation above. You could also create a new session as explained in the above exercises.
