...
We currently do not suggest to run the exercises with docker. If you want to run in demo, test or production environments than the Docker approach is
preferable, but without volume mounts for configuration changes using it for exercises is not recommended.
